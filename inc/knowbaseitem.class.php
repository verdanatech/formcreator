 ...
 * ---------------------------------------------------------------------
 * Verdanatech - arquivo criado e modificado
 */
 ...
 * KnowbaseItem Class
 **/
class PluginFormcreatorKnowbaseItem extends CommonDBVisible
{
    ...
    function showManageForm($options)
    {
        ...
        echo "<tr class='tab_bg_2'><td class='right' width='50%'>";
        $values = [
            'myunpublished' => __('My unpublished articles'),
        ...
        if ($result = $DB->query($query)) {
            ...
            if ($numrows_limit > 0) {
                ...
                if ($output_type == Search::HTML_OUTPUT) {
                    ...
                    if (!Session::getLoginUserID()) {
                        $pager_url = FORMCREATOR_ROOTDOC . "/front/helpdesk.faq.php";
                    }
                    ...
                    if ($output_type == Search::HTML_OUTPUT) {
                        ...
                        if (
                            ...
                            $toadd = Ajax::createIframeModalWindow(
                                'kbshow' . $data["id"],
                                PluginFormcreatorKnowbaseItem::getFormURLWithID($data["id"]),
                                ['display' => false]
                            );
                        } else {
                            $href = " href=\"" . PluginFormcreatorKnowbaseItem::getFormURLWithID($data["id"]) . "\" ";
                        }
                        ...
                        echo Search::showItem(
                            ...
                            "<div class='kb'>$toadd <i class='fa fa-fw $fa_class' title='$fa_title'></i> <a $href>" . Html::resume_text($name, 80) . "</a></div>
                                          <div class='kb_resume'>" .
                                Html::resume_text(
                                    Html::clean(Toolbox::unclean_cross_side_scripting_deep($answer)),
                                    600
                                ) . "</div>",
                            $item_num,
                            $row_num
                        ...
                        echo Search::showItem(
                            $output_type,
                            Html::clean(Toolbox::unclean_cross_side_scripting_deep(html_entity_decode(
                                $answer,
                                ENT_QUOTES,
                                "UTF-8"
                            ))),
    ...
    static function showRecentPopular($type)
    {
        ...
        if ($number > 0) {
            ...
            while ($data = $DB->fetch_assoc($result)) {
                ...
                echo Html::link(Html::resume_text($name, 80), PluginFormcreatorKnowbaseItem::getFormURLWithID($data["id"]), [
